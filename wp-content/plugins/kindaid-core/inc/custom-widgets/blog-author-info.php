...
// ==============================
class Kindaid_Blog_Author_Info extends WP_Widget {

    public function __construct() {
        parent::__construct(
            'kindaid_blog_author_info',
            __('Kindaid Blog Author Info', 'kindaid'),
            array('author_deg' => __('Blog Sidebar Author Image,Author Position,Author Name,Author Social', 'kindaid'))
        );
    }
    ...
    public function widget($args, $instance) {
       echo wp_kses_post( $args['before_widget']);
        ...
        $author_deg = !empty($instance['author_deg']) ? $instance['author_deg'] : '';
        ...
        ?>
        <div class="tp-widget-author text-center">
            ...
            <div class="tp-widget-author-content">
                <?php if(!empty($author_deg)):?>
                <span class="tp-widget-author-subtitle d-inline-block mb-5"><?php echo esc_html($author_deg);?></span>
                <?php endif;?>
                <?php if(!empty($author_title)):?>
                <h3 class="tp-widget-author-title mb-15"><?php echo esc_html($author_title);?></h3>
                <?php endif;?>
    ...
    public function form($instance) {
        ...
        <p>
            <label><?php echo esc_html__('Author Image URL:','kindaid');?></label>
            <input class="widefat upload-image" name="<?php echo esc_attr($this->get_field_name('author_image')); ?>" type="text" value="<?php echo esc_attr($author_image); ?>">
            <button class="button select-media"><?php echo esc_html__('Upload Image','kindaid');?></button>
        </p>

        <p>
            <label><?php echo esc_html__('Author Deg:','kindaid');?></label>
            <textarea class="widefat" rows="4" name="<?php echo esc_attr($this->get_field_name('author_deg')); ?>"><?php echo esc_textarea($author_deg); ?></textarea>
        </p>

        <p>
            <label><?php echo esc_html__('Author Title:','kindaid');?></label>
            <input class="widefat" name="<?php echo esc_attr($this->get_field_name('author_title')); ?>" type="text" value="<?php echo esc_attr($author_title); ?>">
        </p>
            ...
            jQuery(document).ready(function($){
                $('.select-media').on('click', function(e){
                    ...
                    var custom_uploader = wp.media({
                        author_title: 'Select Logo Image',
    ...
    public function update($new_instance, $old_instance) {
        return array(
            'author_image'          => esc_url_raw($new_instance['author_image'] ?? ''),
            'author_deg'   => sanitize_textarea_field($new_instance['author_deg'] ?? ''),
            'author_title'         => sanitize_text_field($new_instance['author_title'] ?? ''),
            'facebook_url'  => esc_url_raw($new_instance['facebook_url'] ?? ''),
            'twitter_url'   => esc_url_raw($new_instance['twitter_url'] ?? ''),
            'website_url'   => esc_url_raw($new_instance['website_url'] ?? ''),
            'instagram_url' => esc_url_raw($new_instance['instagram_url'] ?? ''),
        );
    }
...
// Register widget
function kindaid_register_blog_info_widget() {
    register_widget('Kindaid_Blog_Author_Info');
}
add_action('widgets_init', 'kindaid_register_blog_info_widget');
